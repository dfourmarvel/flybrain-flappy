...
def _read_run(run_dir: Path, network_label: str) -> dict | None:
    ...
    row["censored"] = gen_to_comp is None
...
def analyse_primary(real_scores: np.ndarray, control_scores: np.ndarray) -> PrimaryResult:
    """Mann-Whitney U (two-sided, exact when feasible) + rank-biserial effect size on the
    30 real vs 30 control held-out means (PLAN section 1 primary measure)."""
    real_scores = np.asarray(real_scores, dtype=float)
    control_scores = np.asarray(control_scores, dtype=float)
    n_real, n_control = len(real_scores), len(control_scores)

    result = stats.mannwhitneyu(real_scores, control_scores, alternative="two-sided", method="auto")
    u = float(result.statistic)
    p = float(result.pvalue)

    # rank-biserial = 2U/(n1*n2) - 1 (Wendt 1972); positive => real tends to rank higher.
    rank_biserial = float(2 * u / (n_real * n_control) - 1) if n_real and n_control else float("nan")
    if rank_biserial > 0:
    ...
    ceiling_real = int(np.sum(real_scores >= CEILING_SCORE))
    ceiling_control = int(np.sum(control_scores >= CEILING_SCORE))
    uninformative = bool(
        len(real_scores) and len(control_scores)
        and ceiling_real == len(real_scores) and ceiling_control == len(control_scores)
    )

    return PrimaryResult(
        n_real=n_real, n_control=n_control,
        median_real=float(np.median(real_scores)) if n_real else float("nan"),
        median_control=float(np.median(control_scores)) if n_control else float("nan"),
        iqr_real=iqr(real_scores), iqr_control=iqr(control_scores),
        median_diff=(float(np.median(real_scores) - np.median(control_scores))
                     if n_real and n_control else float("nan")),
        mean_real=float(np.mean(real_scores)) if n_real else float("nan"),
        mean_control=float(np.mean(control_scores)) if n_control else float("nan"),
        ceiling_count_real=ceiling_real,
        ceiling_count_control=ceiling_control,
        u_statistic=u, p_value=p, rank_biserial=rank_biserial, direction=direction,
...
class LogRankResult:
    ...
    median_real: float | None  # None means "not reached" (censored median)
    median_control: float | None


def _median_or_not_reached(times: np.ndarray, censored: np.ndarray) -> float | None:
    """Sample median of the generations-to-competence, treating a censored run as "never
    reached" rather than imputing a value. If more than half the arm is censored, the true
    median is itself censored ("not reached", the conservative Kaplan-Meier reading)."""
    n = len(times)
    if n == 0:
        return None
    ...
    # If the middle observation(s) are censored, the median cannot be pinned down -> not reached.
    mid_lo = (n - 1) // 2
    mid_hi = n // 2
    if cens_sorted[mid_lo] or cens_sorted[mid_hi]:
        return None
    return float((times_sorted[mid_lo] + times_sorted[mid_hi]) / 2.0)
...
def log_rank_test(
    ...
    return LogRankResult(
        ...
        median_real=_median_or_not_reached(time_real, event_real == 0),
        median_control=_median_or_not_reached(time_control, event_control == 0),
    )


def build_survival_arrays(df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    """(time, event) arrays for the secondary measure from a loaded-runs dataframe. time is
    generations_to_competence if reached, else last_generation (censoring time); event is 1 if
    reached, 0 if censored (PLAN section 1: "Censored at the run's budget ... use a
    survival/rank test, not a mean")."""
    if df.empty:
        return np.array([]), np.array([])
    reached = df["generations_to_competence"]
    last_gen = df["last_generation"].fillna(0)
...
def fig_gf_habituation(replay_path: Path, out_path: Path) -> bool:
    """Writes the giant-fiber habituation figure from a replay JSON if it exists AND is a
    real-connectome recording. Returns True if the figure was written, False if skipped."""
...
def render_results_md(
    ...
    lines.append("")
    lines.append(
        f"Mann-Whitney U = {_fmt(primary.u_statistic)}, two-sided p = {_fmt(primary.p_value, 4)}. "
        f"Rank-biserial correlation = {_fmt(primary.rank_biserial)} ({primary.direction})."
    )
    lines.append("")
    if primary.uninformative:
        lines.append(
    ...
    lines.append(
        f"| median generations | {_fmt(secondary.median_real)} | {_fmt(secondary.median_control)} |"
    )
    lines.append(
    ...
    lines.append("")
    lines.append(
        f"Log-rank chi-square = {_fmt(secondary.chi2)}, p = {_fmt(secondary.p_value, 4)}."
    )
    lines.append("")
    ...
    if gf_figure_written:
        ...
        lines.append(
            "4. Giant-fiber habituation figure skipped: `docs-site/replay/best.json` is either "
            "missing or not recorded from the real connectome (`meta.is_real_connectome` is "
            "false), so no real spiking data exists to plot."
        )
    lines.append("")
...
def run_analysis(runs_dir: str | Path, out_dir: str | Path,
    ...
    print(
        f"primary: median real={primary.median_real} control={primary.median_control} "
        f"U={primary.u_statistic} p={primary.p_value} rank_biserial={primary.rank_biserial}"
    )

    time_real, event_real = build_survival_arrays(loaded.real)
    time_control, event_control = build_survival_arrays(loaded.control)
    secondary = log_rank_test(time_real, event_real, time_control, event_control)
    print(
        f"secondary: median_gens real={secondary.median_real} control={secondary.median_control} "
        f"chi2={secondary.chi2} p={secondary.p_value}"
    )
    ...
    gf_written = fig_gf_habituation(replay_path, gf_path)
    ...
    md = render_results_md(loaded, primary, secondary, gf_written, figures_rel)
